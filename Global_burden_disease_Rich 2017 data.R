...
CCG <- read_csv(url("https://opendata.arcgis.com/datasets/4010cd6fc6ce42c29581c4654618e294_0.csv"), col_types = cols(CCG18CD = col_character(),CCG18NM = col_character())) %>%
  select(-c(CCG18CDH, FID)) %>% 
...
GBD_2017_cause_hierarchy <- read_excel("./GBD/IHME_GBD_2017_CAUSE_HIERARCHY_Y2018M11D18.xlsx", col_types = c("text", "text", "text", "text", "text", "numeric", "text", "text"))
 
GBD_cause_codebook <- read_csv("./GBD/IHME_GBD_2017_CODEBOOK_Y2018M11D18.csv", col_types = cols_only(cause_id = col_character(), cause_name = col_character())) %>% 
  filter(cause_id != "Cause ID")

# REI IDs for risk, etiology, and impairment variables
gbd_rei_hierarchy <- read_excel("./GBD/IHME_GBD_2017_REI_HIERARCHY_Y2018M11D18.xlsx", col_types = c("text", "text", "text", "text", "text", "numeric"))
...
GBD_cause_data <- read_csv("./GBD/IHME-GBD_2017_DATA-919f4e67-1.csv", col_types = cols(age = col_character(), cause = col_character(), location = col_character(), lower = col_double(), measure = col_character(), metric = col_character(), sex = col_character(), upper = col_double(), val = col_double(), year = col_number())) %>%
  ...
  bind_rows(read_csv("./GBD/IHME-GBD_2017_DATA-919f4e67-3.csv", col_types = cols(age = col_character(), cause = col_character(), location = col_character(), lower = col_double(), measure = col_character(), metric = col_character(), sex = col_character(), upper = col_double(), val = col_double(), year = col_number())))%>%
  bind_rows(read_csv("./GBD/IHME-GBD_2017_DATA-919f4e67-4.csv", col_types = cols(age = col_character(), cause = col_character(), location = col_character(), lower = col_double(), measure = col_character(), metric = col_character(), sex = col_character(), upper = col_double(), val = col_double(), year = col_number()))) %>% 
  bind_rows(read_csv("./GBD/IHME-GBD_2017_DATA-919f4e67-5.csv", col_types = cols(age = col_character(), cause = col_character(), location = col_character(), lower = col_double(), measure = col_character(), metric = col_character(), sex = col_character(), upper = col_double(), val = col_double(), year = col_number())))

GBD_cause_group <- GBD_2017_cause_hierarchy %>% 
  filter(level == 1)

# Incidence data published for 2017
# Prevalence data published for 2017

# Proportions of morbidity, mortality and dalys due to particular causes ####

Measures_perc <- GBD_cause_data %>% 
...
Measures_perc_level_1 <- Measures_perc %>% 
  filter(level == 1) %>% 
  arrange(cause_outline) %>% 
  mutate(cause = ifelse(nchar(cause) < 40, cause, sub('(.{1,40})(\\s|$)', '\\1\n', cause))) %>%
  mutate(cause = factor(cause, levels =  unique(cause))) %>% 
  select(measure, location, sex, year, age, cause, val) %>% 
  group_by(measure, year, location, sex, age) %>%   spread(measure, val) %>% 
  ungroup()

Area_perc_level_1 <- as.data.frame(Measures_perc_level_1 %>% 
  filter(location == "West Sussex",
         year == max(year)))

paste0("In ", unique(Area_perc_level_1$year) ,", non-communicable disease accounted for ", round(subset(Area_perc_level_1, cause == "Non-communicable diseases" & sex == "Male", select = "YLDs (Years Lived with Disability)")*100,0), "% of the burden of ill health among males and ", round(subset(Area_perc_level_1, cause == "Non-communicable diseases" & sex == "Female", select = "YLDs (Years Lived with Disability)")*100,0), "% of the burden of ill health among females living in ", unique(Area_perc_level_1$location), ".")

paste0("Considering ill health and mortality combined, non-communicable disease accounted for ", round(subset(Area_perc_level_1, cause == "Non-communicable diseases" & sex == "Male", select = "DALYs (Disability-Adjusted Life Years)")*100,0), "% of disability adjusted life years among males and ", round(subset(Area_perc_level_1, cause == "Non-communicable diseases" & sex == "Female", select = "DALYs (Disability-Adjusted Life Years)")*100,0), "% of disability adjusted life years among females.")

Measures_perc_level_2 <- Measures_perc %>% 
  filter(level == 2) %>% 
  arrange(cause_outline) %>% 
  mutate(cause = ifelse(nchar(cause) < 40, cause, sub('(.{1,40})(\\s|$)', '\\1\n', cause))) %>%
  mutate(cause = factor(cause, levels =  unique(cause))) %>% 
  select(measure, location, sex, year, age, cause, cause_outline, cause_id, val, level, Parent_cause) %>% 
  group_by(measure, year, location, sex, age) %>% 
  spread(measure, val) %>% 
  ungroup()

Measures_perc_level_3 <- Measures_perc %>% 
  filter(level == 3) %>% 
  arrange(cause_outline) %>% 
  mutate(cause = ifelse(nchar(cause) < 40, cause, sub('(.{1,40})(\\s|$)', '\\1\n', cause))) %>%
  mutate(cause = factor(cause, levels =  unique(cause))) %>% 
...
# We can use this to get statistics on the contribution of each indicator

# Deaths by cause ####

Deaths_wsx_js <- GBD_cause_data %>% 
  ...
  select(location, sex, Parent_cause, year, age, cause, cause_outline, id, val) %>% 
  rename(value = val) %>% 
  arrange(desc(value)) %>% 
  left_join(Measures_perc_level_3[c("location", "sex", "age", "year", "cause_outline", "DALYs (Disability-Adjusted Life Years)", "Deaths", "YLDs (Years Lived with Disability)","YLLs (Years of Life Lost)")], by = c("location", "sex", "age", "year", "cause_outline"))

r2d3(data = Deaths_wsx_js, script = "./Javascripts/gbd_cause_deaths_bubbles.js")
# to do - I have added a load of data on the proportion of 
...
YLL_wsx_js <- GBD_cause_data %>% 
  filter(year %in% c(2017),
         ...
         sex == "Both",
         location == "West Sussex") %>%  
  left_join(GBD_2017_cause_hierarchy[c("cause_name", "cause_id", "cause_outline", "parent_id", "level")], by = c("cause" = "cause_name")) %>% 
  filter(level == 3) %>% 
  left_join(GBD_2017_cause_hierarchy[c("cause_name", "cause_id")], by = c("parent_id" = "cause_id")) %>% 
  rename(Parent_cause = cause_name) %>% 
  mutate(cause = capwords(cause, strict = TRUE)) %>% 
  mutate(Parent_cause = capwords(Parent_cause, strict = TRUE)) %>% 
  mutate(id = paste0(Parent_cause, ".", cause)) %>% 
  select(Parent_cause, cause, cause_outline, id, val, year, age, sex, location) %>% 
  rename(value = val) %>% 
  arrange(desc(value)) %>% 
  left_join(Measures_perc_level_3[c("location", "sex", "age", "year", "cause_outline", "DALYs (Disability-Adjusted Life Years)", "Deaths", "YLDs (Years Lived with Disability)","YLLs (Years of Life Lost)")], by = c("location", "sex", "age", "year", "cause_outline"))
