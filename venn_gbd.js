 ...
 next = {x: initial.slice(), fx: 0, fxprime: initial.slice()},
     ...
     function getDistanceMatrices(areas, sets, setids) {
         ...
         areas.filter(function(x) { return x.sets.length == 2; })
             ...
             var left = setids[current.sets[0]],
                 ...
                 r1 = Math.sqrt(sets[left].Number / Math.PI),
                 r2 = Math.sqrt(sets[right].Number / Math.PI),
                 distance$$1 = distanceFromIntersectArea(r1, r2, current.Number);
             ...
             // relationship
             var c = 0;
             if (current.Number + 1e-10 >= Math.min(sets[left].Number,
                                                  sets[right].Number)) {
                 c = 1;
             } else if (current.Number <= 1e-10) {
                 c = -1;
             }
     ...
     function constrainedMDSLayout(areas, params) {
         ...
         for (i = 0; i < sets.length; ++i) {
             ...
             circles[set.sets[0]] = {
                 ...
                 radius:  Math.sqrt(set.Number / Math.PI)
             };
         }
     ...
     function greedyLayout(areas, params) {
         ...
         for (var i = 0; i < areas.length; ++i) {
             ...
             if (area.sets.length == 1) {
                 ...
                 circles[set] = {x: 1e10, y: 1e10,
                                 ...
                                 Number: area.Number,
                                 radius: Math.sqrt(area.Number / Math.PI)};
                 setOverlaps[set] = [];
             }
         }
         ...
         for (i = 0; i < areas.length; ++i) {
             ...
             if (current.Number + SMALL$1 >= Math.min(circles[left].Number,
                                                  circles[right].Number)) {
                 weight = 0;
             }

             setOverlaps[left].push ({set:right, Number:current.Number, weight:weight});
             setOverlaps[right].push({set:left,  Number:current.Number, weight:weight});
         }
         ...
         for (set in setOverlaps) {
             if (setOverlaps.hasOwnProperty(set)) {
                 ...
                 for (i = 0; i < setOverlaps[set].length; ++i) {
                     Number += setOverlaps[set][i].Number * setOverlaps[set][i].weight;
                 }

                 mostOverlapped.push({set: set, Number:Number});
             }
         }
     ...
     function lossFunction(sets, overlaps) {
         ...
         for (var i = 0; i < overlaps.length; ++i) {
             ...
             output += weight * (overlap - area.Number) * (overlap - area.Number);
         }
     ...
     function normalizeSolution(solution, orientation, orientationOrder) {
         ...
         for (i = 0; i < clusters.length; ++i) {
             ...
             clusters[i].Number = (bounds.xRange.max - bounds.xRange.min) * (bounds.yRange.max - bounds.yRange.min);
             clusters[i].bounds = bounds;
         }
         clusters.sort(function(a, b) { return b.Number - a.Number; });
     ...
     function scaleSolution(solution, width, height_overlap, padding) {
         var circles = [], setids = [];
...
function chart(selection) {
     ...
     var toremove = {};
       ...
       if ((datum.Number == 0) && datum.sets.length == 1) {
         toremove[datum.sets[0]] = 1;
             ...
             if (data.length > 0) {
                 ...
                 circles = scaleSolution(solution, width, height_overlap, padding);
                 textCentres = computeTextCentres(circles, data);
             }

// Figure out the current label for each set. These can change and D3 won't necessarily update (fixes https://github.com/benfred/venn.js/issues/103)
var labels = {};
   data.forEach(function(datum) {
       if (datum.Risk) {
       labels[datum.sets] = datum.Risk;
       }
      });
             ...
             var pathTween = function(d) {
                 return function(t) {
                     var c = d.sets.map(function(set) {
                         ...
                         if (!start) {
                             start = {x : width/2, y : height_overlap/2, radius : 1};
                         }
                         if (!end) {
                             end = {x : width/2, y : height_overlap/2, radius : 1};
                         }
...
var nodes = svg_overlap.selectAll(".venn-area")
...
// Updated to account for only displaying the text for the first three names, everything else is white, with zero opacity and zero font size.
 if (styled) {
     enterPath.style("fill-opacity", "0")
              .filter(function (d) { return d.sets.length == 1; } )
              .style("fill", function(d) { return d.Risk === 'Burden not attributable to GBD risk factors' ? '#000' : colours(d.sets); })
              .style("fill-opacity", ".25");
     enterText
              .style("fill", function(d) { return d.Risk === 'Burden not attributable to GBD risk factors' ? '#000' : colours(d.sets); })
              .style("fill-opacity", function(d) { return d.sets.length == 1 ? '1' : "0"; })
              .style("font-size", function(d) { return d.sets.length == 1 ? '1rem' : "0px"; });
     }
